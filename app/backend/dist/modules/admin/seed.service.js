...
let SeedService = SeedService_1 = class SeedService {
    ...
    async onModuleInit() {
        const email = process.env.ADMIN_LOGIN_EMAIL ??
            this.config.get('mail.adminEmail') ??
            'admin@bioecolab.ro';
        ...
        await this.clients.create({
            ...
            cui: '-',
            email,
            role: client_schema_1.UserRole.ADMIN,
            status: client_schema_1.AccountStatus.ACTIV,
            passwordHash: await bcrypt.hash(password, 10),
        });
        this.logger.warn(`Cont admin creat: ${email} / ${password} — schimbă parola în producție (ADMIN_PASSWORD).`);
    }
};
